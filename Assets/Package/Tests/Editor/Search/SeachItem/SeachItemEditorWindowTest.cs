...
{
    public class SeachItemEditorWindowTest
    {
        [Test]
        ...
        void CheckClass(IList<SearchLogic> searchLogics)
        {
        ...
        List<Type> GetInterfaces(List<Type> types)
        {
        ...
        void CheckClass<TImplement>(IList<SearchLogic> searchLogics, List<Type> interfaces)
        ...
        List<Type> GetClasses(List<Type> interfaces, Assembly assembly)
        {
            var list = new List<Type>();
        ...
        void CheckClass(IList<SearchLogic> searchLogics, List<Type> implements)
        {
            if(searchLogics==null)
            {
                Assert.Fail($"Invalid param. searchLogics is null.");
                return;
            }

            if (implements == null)
            {
                Assert.Fail($"Invalid param. implements is null.");
                return;
            }
            ...
            foreach(var t in implements)
            {
                if (!hash.Contains(t))
                {
                    ...
                    {
                        hash.Add(t);
                        Assert.Fail($"Not implement class. Type:{t.Name}, {new Uri(t.Assembly.CodeBase).AbsolutePath}, {t.AssemblyQualifiedName}");
